...
class CSABlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        ### YOUR CODE HERE ###
        # key query and value are defined by using the number
        # of embeddings defined
        ...
        ### YOUR CODE HERE ###
        # masking is done to ensure that the attention weight is 0
        # for elements in the attention matrix above the diagonal
        # and elements in lower triangle have attention weight = 1
        # this is to ensure only previous tokens are considered
    ...
    def forward(self, x, layer_past=None):
        ...
        # Q, K, V for all heads
        ### YOUR CODE HERE ###
        # computing queries, keys and values in multiple head case
        # B, L, self.n_head, C // self.n_head is the projection layer for multi-head case
        ...
        # attention dropout
        ### YOUR CODE HERE ###
        # attention score is calculated by taking dot product of queries and keys,
        # and normalized by dividing by the square root of the key dimension.
        # then masking of future tokens is done
        # attention score of masked token is set as minus infinity
        # softmax and dropout are applied
        attn_save = torch.matmul(queries, keys.transpose(-2, -1)) / (keys.size(-1) ** 0.5)
        attn_save = attn_save.masked_fill(self.mask[:, :, :L, :L] == 0, float('-inf'))
        attn_save = F.softmax(attn_save, dim=-1)
        attn_save_with_dropout = self.attn_drop(attn_save)
        ### YOUR CODE HERE ###

        # Apply the attention to the values; Combine all head outputs
        ### YOUR CODE HERE ###
        # all heads are concatenated
        y = torch.matmul(attn_save_with_dropout, values).transpose(1, 2).contiguous().view(B, L, C)
        ### YOUR CODE HERE ###
